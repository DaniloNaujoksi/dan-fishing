# Schneidet die zwoelf Fische des ersten Blattes neu aus.
#
# Die Fenster stammen diesmal aus einer Messung der Inhaltsspalten, nicht aus
# Schaetzung: Beim Hecht fehlten dadurch 30 Pixel Schwanzflosse. Jedes Fenster
# bekommt zusaetzlich einen Rand von 6 Pixeln, damit nichts knapp anliegt.

Add-Type -AssemblyName System.Drawing

$source = 'C:\Users\Daniel\workspaces\mizuumi-fishing\Graphics\fischarten_ART.png'
$outDir = 'C:\Users\Daniel\workspaces\mizuumi-fishing\Graphics\cut'
New-Item -ItemType Directory -Force -Path $outDir | Out-Null

$pad = 6

# id, x1, x2, y1, y2  (gemessene Grenzen)
$cells = @(
    @('roach',           42,  262, 135, 282),
    @('perch',          326,  565, 135, 282),
    @('tench',          633,  860, 135, 282),
    @('carp',           909, 1170, 135, 282),
    @('rainbow_trout', 1218, 1482, 135, 282),

    @('pike',            31,  394, 434, 575),
    @('zander',         456,  771, 434, 575),
    @('koi',            833, 1151, 434, 575),
    @('eel',           1203, 1488, 434, 575),

    @('catfish',         71,  492, 720, 896),
    @('golden_trout',   553,  861, 720, 896),
    @('moon_carp',      963, 1348, 720, 896)
)
...
foreach ($cell in $cells) {
    $id = [string]$cell[0]
    $cx = [Math]::Max(0, [int]$cell[1] - $pad)
    $cy = [Math]::Max(0, [int]$cell[3] - $pad)
    $cw = [Math]::Min($srcW - $cx, [int]$cell[2] - [int]$cell[1] + 1 + $pad * 2)
    $ch = [Math]::Min($srcH - $cy, [int]$cell[4] - [int]$cell[3] + 1 + $pad * 2)

    $tile = New-Object byte[] ($cw * $ch * 4)
    for ($y = 0; $y -lt $ch; $y++) {
        [System.Array]::Copy($bytes, ($cy + $y) * $stride + $cx * 4, $tile, $y * $cw * 4, $cw * 4)
    }
    ...
    $bmp.Save((Join-Path $outDir "fish_$id.png"), [System.Drawing.Imaging.ImageFormat]::Png)
    $bmp.Dispose()

    # Randberuehrung melden: Wenn der Inhalt bis an den Fensterrand reicht,
    # wurde vermutlich etwas abgeschnitten.
    $touches = @()
